...
public class AutoPlaylist extends Playlist implements Parcelable {
	...
	public Observable<List<Song>> generatePlaylist(MusicStore musicStore,
		...
		Timber.e("Getting Autoplaylist songs for " + playlistName);
        ...
        Observable<List<Song>> truncated = truncateFilteredSongs(finalFiltered, playCountStore);
	...
	private static void sortSongListByField(List<Song> songs, @AutoPlaylistRule.Field int field, boolean ascending,
	                                        PlayCountStore playCountStore) {
	...
	private static Comparator<Song> getSortComparator(@AutoPlaylistRule.Field int field,
	                                                  PlayCountStore playCountStore) {
		switch (field) {
